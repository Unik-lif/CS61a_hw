...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            originant = place.ant
            if originant.is_container is True and originant.ant_contained is None and self.is_container is False:
                originant.ant_contained = self
            elif originant.is_container is False and self.is_container is True and self.ant_contained is None:
                place.ant = self
                self.ant_contained = originant
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        counter = 0
        placeptr = self.place
        while placeptr.is_hive is False:
            if placeptr.bees and self.min_range <= counter <= self.max_range:
                return random_bee(placeptr.bees)
            placeptr = placeptr.entrance
            counter += 1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for i in range(len(bees)):
            if bees[i].health <= amount + flag:
                self.place.bees[i - counter].reduce_health(amount + flag)
                counter += 1
            else:
                self.place.bees[i - counter].reduce_health(amount + flag)
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt stop bees with its great health"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eat bees at his place."""
    ...
    def action(self, gamestate):
        """Eat a bee and chew for a while"""
...
class TankAnt(ContainerAnt):
    """TankAnt provides denfense as well as 1 damage to all the bees"""
    ...
    def action(self, gamestate):
        ...
        bees = self.place.bees[:]
        for i in range(len(bees)):
            if bees[i].health <= self.damage:
                self.place.bees[i - counter].reduce_health(self.damage)
                counter += 1
            else:
                self.place.bees[i - counter].reduce_health(self.damage)
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is waterproof, and can attack bees flying over the water."""
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.hasqueen is False:
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        placeptr = self.place.exit
        while placeptr is not None:
            if placeptr.ant:
                placeptr.ant.buff()
                if placeptr.ant.is_container is True and placeptr.ant.ant_contained:
                    placeptr.ant.ant_contained.buff()
            placeptr = placeptr.exit
        # END Problem 12
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        gobackward = self.place.entrance
        # Extra credit: Special handling for bee direction
        if self.blocked():
            self.sting(self.place.ant)
        elif self.slowtime:
            if gamestate.time % 2 == 0 and self.backward == 0:
                self.move_to(destination)
            elif gamestate.time % 2 == 0 and self.backward:
                if gobackward.is_hive is False:
                    self.move_to(gobackward)
                self.backward -= 1
            self.slowtime -= 1
        elif self.backward:
