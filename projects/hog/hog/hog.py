...
def roll_dice(num_rolls, dice=six_sided):
    ...
    flag, sum = 0, 0
    while num_rolls:
        num_rolls, dice_ret = num_rolls - 1, dice()
        if dice_ret == 1:
            sum, flag = 1, 1
        elif flag == 0:
            sum += dice_ret
    ...
    def announce(score0, score1):
        if who == 0:
            if score0 - last_score > running_high:
                ...
                return announce_highest(who, last_score=score0, running_high=running_temp)
        else:
            if score1 - last_score > running_high:
                ...
                return announce_highest(who, last_score=score1, running_high=running_temp)
    return announce
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    >>> averaged_dice = make_averaged(roll_dice, 1000) # bind averaged_dive function with roll_dice, then insert functions into roll_dice with averaged_dice, loop for trials_count times.
    ...
    def average_function(*args):
        sum = 0
        trials_temp = trials_count
        while trials_temp:
            trials_temp, sum = trials_temp - 1, sum + original_function(*args)
...
def final_strategy(score, opponent_score):
    ...
    My strategy:
    Hog_pile_strategy is a good way if you are left behind.

    At first, I would prefer to choose picky_piggy_strategy.
    ...
    if score - opponent_score <= 5:
        ...
        if score - opponent_score > 5:
            return picky_piggy_strategy(score, opponent_score, cutoff=5, num_rolls=4)
        elif opponent_score - score > 10:
            return picky_piggy_strategy(score, opponent_score, cutoff=7, num_rolls=7)
        else:
