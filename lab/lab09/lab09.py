...
class Account:
    ...
    def __repr__(self):
        deposit_counter, withdraws_counter = 0, 0
        for i in self.transactions:
            if i[0] == 'deposit':
                deposit_counter += 1
            elif i[0] == 'withdraw':
                withdraws_counter += 1
        return f'Accountholder: {self.holder}, Deposits: {deposit_counter}, Withdraws: {withdraws_counter}'
...
def trade(first, second):
    ...
    while equal_prefix() is False and m <= len(first) and n <= len(second):
...
def insert(link, value, index):
    ...
    element = Link(value)
    counter = 1
    if index == 0: # case is strange when index is 0.
        ...
        while counter < index:
            pre_ptr = link_ptr
            link_ptr = link_ptr.rest
            if link_ptr == Link.empty:
                raise IndexError('Out of bounds!')
            counter += 1
        pre_ptr.rest = element
        element.rest = link_ptr
    ...
    def path_finder(t):
        ...
        if t.is_leaf():
            ...
            for subt in t.branches:
                ret += insert_list(path_finder(subt), t.label)
            return ret
